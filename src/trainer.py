...
from .data_utils import EarlyStoppingCallback, pretokenize_supervised_dataset, smart_collate_fn
from .logger import CSVMetricsWriter, SystemMonitor, setup_logging
...
# ==============================================================================
# 輔助與對齊元件 (_NoOpMetricsWriter, _DistributedEvalSampler)
# ==============================================================================
...
# ==============================================================================
# 進階訓練器 (AdvancedNagatoSakuraTrainer)
# ==============================================================================
...
class AdvancedNagatoSakuraTrainer:
    ...
    def _log_system_environment(self):
        """格式化記錄系統環境與硬體檢查 (階段 1/6)"""
        
        self.logger.info("════════════════ [階段 1/6] 系統環境與硬體檢查 ════════════════")
        py_ver = sys.version.split()[0]
        torch_ver = torch.__version__
        cuda_info = f"CUDA: {torch.version.cuda}" if torch.cuda.is_available() else "CUDA: 不可用"
        self.logger.info(f"環境基礎 - Python: {py_ver} | PyTorch: {torch_ver} | {cuda_info}")
        if torch.cuda.is_available():
            for gpu_idx in range(torch.cuda.device_count()):
                props = torch.cuda.get_device_properties(gpu_idx)
                name = torch.cuda.get_device_name(gpu_idx)
                mem_gb = props.total_memory / (1024**3)
                self.logger.info(f"GPU[{gpu_idx}] 設備 - {name} ({mem_gb:.1f}GB)")
                
            bf16_ok = hasattr(torch.cuda, "is_bf16_supported") and torch.cuda.is_bf16_supported()
            self.logger.info(f"硬體加速 - BF16支援: {'可用' if bf16_ok else '不可用'} | 混合精度: 可用")
    ...
    def __init__(
        ...
        self.logger.info(f"分散式狀態 - enabled: {self.is_distributed}, rank: {self.rank}, local_rank: {self.local_rank}, world_size: {self.world_size}")
    ...
    def _set_scheduler_step_offset(self, raw_offset: int, source: str) -> int:
        ...
        self.logger.warning(f"偵測到非常規 scheduler step offset({offset})，來源={source}，已正規化為 {normalized}")
    ...
    def _apply_resume_lr_scale(self, optimizer, scheduler, scale: float, reason: str) -> None:
        ...
        self.logger.info(f"已套用續訓學習率縮放({reason}) - scale={scale_value:.6f}, current_lr={lr_display}")
        ...
        try:
            ...
            if isinstance(total_steps, int) and total_steps > 0:
                ...
                try:
                    ...
                    current_lr = (
                        scheduler.get_last_lr()[0] if hasattr(scheduler, "get_last_lr") else None
                    )
                    if current_lr is not None:
                        self.logger.info(
                    ...
                    self.logger.warning(f"學習率調度器對齊失敗({reason})，且 capped fallback 失敗: {fallback_error}")
                    return
    ...
    # --------------------------------------------------------------------------
    # 模型與優化器初始化 (Model & Optimizer Setup)
    # --------------------------------------------------------------------------

    def initialize_model(self):
        """初始化模型"""
        
        self.logger.info("════════════════ [階段 4/6] 模型初始化與配置 ════════════════")
        ...
        try:
            ...
            if getattr(self.model_config, "use_compile", False):
                try:
                    base_model.enable_torch_compile(
                        mode=getattr(self.model_config, "compile_mode", "default"),
                        dynamic=True,
                    )
                except Exception as e:
                    self.logger.warning(f"torch.compile 初始化失敗: {e}，使用 Eager 模式")
            ...
            self.logger.info(f"參數組成: Embedding {embedding_params / 1e6:.2f}M + 非Embedding {non_embedding_params / 1e6:.2f}M")
    ...
    def _load_supervised_data_file(self, data_file: str, dataset_name: str) -> List[Dict[str, str]]:
        ...
        self.logger.info(f"{dataset_name}載入完成 - 來源檔案: {len(source_files)}, 有效: {len(valid_data)}, 格式錯誤: {invalid_count}")
        return valid_data

    def _count_overlapping_samples(self, train_data: List[Dict[str, str]], eval_data: List[Dict[str, str]]) -> int:
    ...
    # --------------------------------------------------------------------------
    # 資料準備與 Dataset 建立 (Data Preparation & Dataset Creation)
    # --------------------------------------------------------------------------
    ...
    def prepare_data_and_tokenizer(
        ...
        tokenizer_enable_zh_common: bool = True,
        tokenizer_enable_zh_yi: bool = False,
        tokenizer_enable_zh_bing: bool = False,
        tokenizer_enable_ja: bool = True,
        tokenizer_enable_emoji: bool = True,
        tokenizer_enable_symbols: bool = True,
        tokenizer_enable_programming: bool = False,
    ) -> Tuple[List[Dict[str, str]], Optional[List[Dict[str, str]]]]:
        """準備訓練/評估數據並訓練分詞器。"""
        
        self.logger.info("════════════════ [階段 2/6] 訓練與評估數據載入 ════════════════")
        ...
        self.logger.info("════════════════ [階段 3/6] 分詞器 (Tokenizer) 建置與載入 ════════════════")
        self.tokenizer_manager.prepare_tokenizer(
            ...
            enable_zh_common=tokenizer_enable_zh_common,
            enable_zh_yi=tokenizer_enable_zh_yi,
            enable_zh_bing=tokenizer_enable_zh_bing,
            enable_ja=tokenizer_enable_ja,
            enable_emoji=tokenizer_enable_emoji,
            enable_symbols=tokenizer_enable_symbols,
            enable_programming=tokenizer_enable_programming,
        )
    ...
    def create_datasets(
        ...
        self.logger.info("════════════════ [階段 5/6] Pretokenize 預分詞與數據集準備 ════════════════")
        ...
        def _finalize_datasets(train_dataset: Dataset,eval_dataset: Optional[Dataset],stage_name: str) -> Tuple[Dataset, Optional[Dataset]]:
            self._audit_dataset_unk_tokens(train_dataset,
        ...
        if not bool(enable_pretokenize):
            ...
            if fixed_eval_data is not None:
                ...
                self.logger.info(f"使用固定評估集(即時分詞) - 訓練集: {len(train_dataset)}, 評估集: {len(eval_dataset)}")
            ...
            if eval_split_ratio > 0 and eval_split_ratio < 1.0 and len(dataset) > 1:
                ...
                self.logger.info(f"數據集分割完成(即時分詞) - 訓練集: {len(train_dataset)}, 評估集: {len(eval_dataset)}")
        ...
        if pretokenize_num_proc is None or pretokenize_num_proc <= 0:
            effective_num_proc = 1
        else:
            effective_num_proc = max(1, int(pretokenize_num_proc))
        ...
        self.logger.info(
            "Pretokenize 設定 - batch_size: %s, num_proc: %s (%s), cache: %s",
            pretokenize_batch_size,
            effective_num_proc,
            "Rust Rayon 原生滿核加速" if effective_num_proc == 1 else "多進程分詞",
        ...
        def _pretokenize_rows(rows: List[Dict[str, Any]], desc: str, split_name: str) -> Dataset:
            try:
                ...
                self.logger.warning(f"{desc} 失敗，回退為即時分詞路徑: {e}")
        ...
        if fixed_eval_data is not None:
            ...
            self.logger.info(f"使用固定評估集 - 訓練集: {len(train_dataset)}, 評估集: {len(eval_dataset)}")
            return _finalize_datasets(train_dataset, eval_dataset, "pretokenize_fixed_eval")

        dataset = Dataset.from_list(data_list)
        if eval_split_ratio > 0 and eval_split_ratio < 1.0 and len(dataset) > 1:
            split_datasets = dataset.train_test_split(test_size=eval_split_ratio, shuffle=True, seed=42)
            train_dataset = _pretokenize_rows(list(split_datasets["train"]), "Pretokenize train", "train")
            eval_dataset = _pretokenize_rows(list(split_datasets["test"]), "Pretokenize eval", "eval")
    ...
    def _audit_dataset_unk_tokens(self,dataset: Dataset,dataset_name: str,max_samples: int = 4096,fail_on_unk: bool = False,) -> None:
        tokenizer = self.tokenizer_manager.transformers_tokenizer
    ...
    def setup_training_components(
        ...
        if dataloader_num_workers is None or dataloader_num_workers < 0:
            if sys.platform == "win32":
                num_workers = 0  # Windows 下單行程直接存取記憶體映射 Arrow Dataset 避免 spawn 行程重複 import 模組與 IPC 延遲
            else:
        ...
        if fused_available:
            ...
            self.logger.info("⚡ 啟用 CUDA Fused AdamW 優化器核心 (極速更新與顯存省電)")

        optimizer = AdamW(optimizer_grouped_parameters, **optimizer_kwargs)

        # 學習率調度器
        effective_epochs = max(1, int(scheduler_target_epochs or num_epochs))
        updates_per_epoch = max(1, int(math.ceil(len(train_dataloader) / max(1, gradient_accumulation_steps))))
    ...
    def train_epoch(
        ...
        for step, batch in enumerate(progress_bar):
            ...
            if segment_ids is not None:
                segment_ids = segment_ids.to(self.device, non_blocking=True)

            batch_tokens = int((labels != -100).sum().item())
            epoch_tokens += batch_tokens
            metrics_window_tokens += batch_tokens
            is_last_batch = step == (total_steps - 1)
            should_sync_gradients = (accumulated_steps + 1) >= gradient_accumulation_steps or is_last_batch
            sync_context = contextlib.nullcontext()
            using_no_sync = (self.is_distributed and hasattr(self.model, "no_sync") and not should_sync_gradients)
            if using_no_sync:
            ...
            with sync_context:
                ...
                if not self._all_ranks_true(local_loss_valid):
                    ...
                    divergence_info = getattr(
                        getattr(self.model, "model", self.model), "last_divergence_info", None
                    )
                    detail_msg = f" (發散源: {divergence_info})" if divergence_info else ""
                    if self.is_main_process:
                        self.logger.warning(f"檢測到無效損失，跳過此批次{detail_msg}: {raw_loss}")
                        self.metrics_writer.log_event(
                            {
                                "epoch": self.current_epoch + 1,
                                "global_step": self.global_step,
                                ...
                                "severity": "WARNING",
                                "message": f"loss is nan/inf or missing{detail_msg}",
                                "value": str(raw_loss),
                            }
                        )
                    accumulated_steps = 0
            ...
            if accumulated_steps >= gradient_accumulation_steps:
                ...
                progress_bar.set_postfix_str(f"loss: {loss_val:.4f} | lr: {current_lr:.1e} | step: {self.global_step}")
        ...
        if num_batches == 0:
            raise RuntimeError("本 epoch 沒有任何有效批次，訓練已停止。請檢查分詞器編碼、數據格式與 collate_fn。")
        ...
        if total_seen_batches > 0:
            ...
            if invalid_ratio > 0.3:
                if self.is_main_process:
                    self.logger.warning(f"偵測到較高無效批次比例: {invalid_ratio:.1%} ({invalid_batches}/{total_seen_batches})")
                    self.metrics_writer.log_event(
                        {
                            "epoch": self.current_epoch + 1,
                            "global_step": self.global_step,
    ...
    # --------------------------------------------------------------------------
    # 評估與指標計算 (Evaluation & Metrics Computation)
    # --------------------------------------------------------------------------
    ...
    # --------------------------------------------------------------------------
    # Checkpoint 儲存與 Resume (Checkpoint & Resume Management)
    # --------------------------------------------------------------------------
        ...
        try:
            ...
            if checkpoint_tokenizer_hash and current_tokenizer_hash:
                if checkpoint_tokenizer_hash != current_tokenizer_hash:
                    self.logger.error("檢查點 tokenizer 與當前 output tokenizer 不一致，為避免詞表語義錯位，已拒絕恢復。")
                    self.logger.error(f"checkpoint tokenizer: {checkpoint_tokenizer_path}, sha256={checkpoint_tokenizer_hash}")
                    self.logger.error(f"current tokenizer: {current_tokenizer_path}, sha256={current_tokenizer_hash}")
            ...
            if (
                ...
                self.logger.error(f"checkpoint vocab_size({checkpoint_vocab_size}) 與當前 tokenizer vocab_size({current_vocab_size}) 不一致，已拒絕恢復。")
                self.logger.error("請改用 --no_resume 開新 run（並搭配 --force_retrain_tokenizer）。")
                return False
            ...
            if restore_training_state and state_path.exists():
                ...
                ckpt_epoch_from_name = CheckpointManager._parse_epoch_from_name(
                    checkpoint_path.name
                )
                if (
                    ckpt_epoch_from_name is not None
                    and self.current_epoch == ckpt_epoch_from_name - 1
                ):
                ...
                if (
                    state_tokenizer_hash
                    and checkpoint_tokenizer_hash
                    and state_tokenizer_hash != checkpoint_tokenizer_hash
                ):
                    self.logger.warning("training_state 記錄的 tokenizer hash 與 checkpoint tokenizer 檔案不一致，請確認檔案是否被外部修改。")
                ...
                if (
                    ...
                    self.logger.error(f"training_state tokenizer_vocab_size({state_tokenizer_vocab}) 與當前 tokenizer vocab_size({current_vocab_size}) 不一致，已拒絕恢復。")
                    self.logger.error("請改用 --no_resume 開新 run（並搭配 --force_retrain_tokenizer）。")
                    return False
                ...
                if scheduler_needs_alignment:
                    reason = (
                        "+".join(scheduler_alignment_reasons)
                        if scheduler_alignment_reasons
                        else "unknown"
                    )
                ...
                self.logger.info(
                    "已啟用 model-only 恢復：略過 optimizer/scheduler/scaler 與 step/epoch 狀態"
                )
            ...
            if state_dict is not None:
                ...
                if (
                    checkpoint_embed is not None
                    and tuple(checkpoint_embed.shape) != current_embed_shape
                ):
                    self.logger.error(f"checkpoint embed_tokens shape={tuple(checkpoint_embed.shape)} 與當前模型 shape={current_embed_shape} 不一致，已拒絕恢復。")
                    self.logger.error("請改用 --no_resume 開新 run（並搭配 --force_retrain_tokenizer）。")
                    return False

                if (
                    checkpoint_lm_head is not None
                    and tuple(checkpoint_lm_head.shape) != current_lm_head_shape
                ):
                    self.logger.error(f"checkpoint lm_head shape={tuple(checkpoint_lm_head.shape)} 與當前模型 shape={current_lm_head_shape} 不一致，已拒絕恢復。")
                    self.logger.error("請改用 --no_resume 開新 run（並搭配 --force_retrain_tokenizer）。")
                    return False
    ...
    def _export_training_summary(
        ...
        best_record = (
            min(finite_eval_records, key=lambda x: float(x["eval_loss"]))
            if finite_eval_records
            else None
        )
    ...
    def train(
        ...
        self.eval_medium_max_tokens = max(
            self.eval_short_max_tokens + 1, int(eval_medium_max_tokens)
        )

        self.logger.info("════════════════ [階段 6/6] 開始模型訓練 ════════════════")
        ...
        train_dataloader, optimizer, scheduler, scaler, train_sampler = (
        ...
        if (not auto_match_resume_lr) and (
            not math.isfinite(resume_lr_scale) or resume_lr_scale <= 0.0
        ):
        ...
        if resume_from_checkpoint:
            ...
            if self.is_main_process:
                if resume_checkpoint:
                    ...
                    if resolved_resume_checkpoint is None:
                        self.logger.warning(f"指定的 resume_checkpoint 不存在或不可用: {resume_checkpoint}")
                    else:
            ...
            if latest_checkpoint and self.load_checkpoint(
                ...
                self.is_main_process
                and (
                    (not auto_match_resume_lr and abs(resume_lr_scale - 1.0) >= 1e-12)
                    or auto_match_resume_lr
                )
            ...
            for epoch in range(self.current_epoch, num_epochs):
                ...
                if math.isfinite(epoch_loss):
                    if not math.isfinite(run_best_train_loss):
                        ...
                        train_improved_this_run = (
                            run_best_train_loss - epoch_loss
                        ) >= run_best_improve_delta
                ...
                if run_eval:
                    ...
                    if should_log_details:
                        self.logger.info(
                            f"評估結果 - Loss: {eval_loss:.4f}, Perplexity: {perplexity:.2f}"
                        )
                ...
                if self.early_stopping and early_stop_score is not None:
                    ...
                    self.early_stopping
                    and should_log_details
                    and early_stopping_monitor == "eval_loss"
                ):
