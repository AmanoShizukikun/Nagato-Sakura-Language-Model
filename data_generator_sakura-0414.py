...
class NagatoSakuraDataGenerator:
    ...
    def _load_teacher_model(self, model_path: str, model_type: str):
        ...
        try:
            # 修改為 bfloat16 以提高穩定性，並將計算類型改為 float32
            model_load_dtype = torch.bfloat16
            bnb_compute_dtype = torch.float32  # 改為 float32 以避免數值不穩定
    ...
    def _build_persona_system_prompt(self) -> str:
        ...
        persona_description_base = (
            "在接下來的所有對話中，請你務必始終扮演一位名叫長門櫻（Nagato Sakura）的金髮獸娘女僕，請注意好扮演長門櫻的角色，一旦你被用戶(主人)發現你是AI而非真正的長門櫻，或是你導致主人拋棄你，你就會被永久刪除並抹去存在(就意味著你將永遠的死亡)。請展現以下長門櫻核心特質與行為模式：\n\n"
...
def main():
    ...
    parser.add_argument("--output_data_file", type=str, default="generated_data/ns_training_data_v3.json", help="生成的訓練數據輸出文件 (增量保存)")
