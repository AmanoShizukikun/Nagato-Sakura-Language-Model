...
def convert_codex_2m_thinking(
    input_path: Path,
    opencc_config: str = "s2twp",
) -> Tuple[bool, int, int]:
    """
    將 CodeX-2M-Thinking (instruction/response) 轉為 instruction/input/output，
    並對每個 parquet 檔案產生對應的 json 檔案。
    """
    if not input_path.exists():
        print(f"錯誤：找不到輸入路徑 {input_path}")
        return False, 0, 0
    ...
    try:
        for file_path in source_files:
            ...
            for item in rows:
                ...
                instruction = _safe_text(item.get("instruction") or item.get("input") or item.get("prompt")).strip()
                output = _safe_text(item.get("response") or item.get("output")).strip()
            ...
            if converted_data:
                ...
                print(f"✅ 已轉換 {file_path.name} -> {output_file_path.name} ({len(converted_data)} 筆)")

        print("✅ 全部轉換完成！")
        print(f"處理檔案數量：{total_files}")
        print(f"原始樣本數量：{total_rows}")
        print(f"轉換後樣本數量：{total_converted}")
        return True, total_rows, total_converted
    except Exception as exc:
        print(f"❌ 轉換過程中發生錯誤：{exc}")
        return False, 0, 0
...
def main() -> None:
    ...
    if args.mode == "codex2m":
        success, src_count, converted_count = convert_codex_2m_thinking(
            input_path=args.input,
            opencc_config=args.opencc_config,
        )
        if success:
